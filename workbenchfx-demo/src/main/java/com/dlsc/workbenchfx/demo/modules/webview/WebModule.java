package com.dlsc.workbenchfx.demo.modules.webview;
...
public class WebModule extends WorkbenchModule {
    ...
    public WebModule(String name, Ikon icon, String url) {
        ...
        // setup toolbar
        ToolbarItem back = new ToolbarItem(new FontIcon(MaterialDesign.MDI_CHEVRON_LEFT), event -> webEngine.executeScript("history.back()"));
        ToolbarItem forward = new ToolbarItem(new FontIcon(MaterialDesign.MDI_CHEVRON_RIGHT), event -> webEngine.executeScript("history.forward()"));
        ToolbarItem home = new ToolbarItem(new FontIcon(MaterialDesign.MDI_HOME), event -> webEngine.load(url));
        ToolbarItem reload = new ToolbarItem(new FontIcon(MaterialDesign.MDI_REFRESH), event -> webEngine.reload());
        getToolbarControlsLeft().addAll(back, forward, home, reload, new ToolbarItem(browserUrl));

        ToolbarItem increaseSize = new ToolbarItem(new Group(increaseFontIcon), event -> browser.setFontScale(browser.getFontScale() + FONT_SCALE_INCREMENT));
        ToolbarItem decreaseSize = new ToolbarItem(new Group(decreaseFontIcon), event -> browser.setFontScale(browser.getFontScale() - FONT_SCALE_INCREMENT));
        getToolbarControlsRight().addAll(increaseSize, decreaseSize);

        // update text field with url every time the url of the webview changes
        webEngine.documentProperty().addListener(
                observable -> {
                    Document document = webEngine.getDocument();
                    if (!Objects.isNull(document) && !Strings.isNullOrEmpty(document.getDocumentURI())) {
