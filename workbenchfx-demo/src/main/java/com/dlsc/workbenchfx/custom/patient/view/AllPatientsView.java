...
public class AllPatientsView extends BorderPane implements ViewMixin {
    ...
    public void initializeParts() {
        ...
        pagination.setPageFactory(param -> new PatientView(cabinet.getAllPatients().get(param), translator));
    }

    @Override
    public void setupValueChangedListeners() {
        cabinet.selectedPatientProperty()
               .addListener((observable, oldValue, newValue) -> pagination.setCurrentPageIndex(cabinet.getAllPatients().indexOf(newValue)));

        pagination.currentPageIndexProperty()
                  .addListener((observable, oldValue, newValue) -> cabinet.setSelectedPatient(cabinet.getAllPatients().get(newValue.intValue())));
    }
