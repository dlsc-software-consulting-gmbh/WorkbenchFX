...
We thought about using Test Driven Development (TDD), but beause the framework is new, the API was not clear from the beginning.
...
However, when there were some test cases that were very data driven.
// TODO: add source and maybe code snippet of data driven testing with junit?
JUnit 5 does have data driven testing capabilities, but the way tests have to be implemented to be data driven is not very readable.
// TODO: add source
...
At first, we proceeded to write the data driven tests in JUnit, but the tests looked really bad.
They had lots of duplicated code and therefore were not very maintainable.
// TODO: include image
