...
At the start of the project we created two persona to focus on: SteffiFX and Anna (see <<persona>>).
After the first meeting with our customer, we discovered together even a third persona, which led to additional features and changed the scope of our project.
At first we thought, that it would be sufficient to design an API for long time JavaFX programmers, but then we had to especially keep an eye on developers with rare or even no experience in frontend development.
...
=== Applications
The goal of this analysis is to look at the structure of different applications and to find best practices amongst them.
// TODO: keep the well done, dump bad things
...
<<img-photoshop>> below gives a short impression of the program and <<characteristics-photoshop>> after the image shows our findings.
...
* Usually there are some drawers on the left and right of the application, which can collapse.
...
=== User Stories
In the beginning of the project we decided to make a workshop with our customer.
Its purpose was to define persona (see <<_persona>>) and based on them to create user stories.

We followed a specific pattern when writing our user stories:
We started always with the name of the persona:
____
As Anna...
____
...
____
concluding with the reason why:
____
...so that I am not confused while working and can keep a productive workflow.
____

This way we could instantly verify if a feature was really necessary and useful or not.
Because the story needed to fit in the context and was not just a random requirement which came in mind, without verification cite:[cohn_user_2004].

// TODO: Image of Dieter?
...
We felt, that Google's https://material.io/design/[Material Design] guidelines are a pretty good starting point since their flat design approach is being used a lot in applications and they are providing the guidelines open source cite:[noauthor_design_nodate].
...
The way those tabs are styled adapts mainly to tabs with only one single text cite:[noauthor_tabs_nodate].
...
In the end we decided to rely on the Material Design naming convention of colors and to use them as defined cite:[noauthor_color_nodate].
...
This way we have not to worry about other, future developers not knowing on how the colors are used and how to use them cite:[noauthor_color_nodate].
...
In the <<_user_stories,user story workshop>> with our customer where we defined the persona and afterwards the user stories to define the problems which our API should solve.
