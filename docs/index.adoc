...
== Introduction
What is WorkbenchFX?
WorkbenchFX is an out of the box, open source solution to build large applications from multiple views, called modules.
It offers good user experience and a beautiful design.

A developer start often with building a view to show his customer first progress.
...
It takes the views, and wraps the infrastructure around it.
It does all the hard work for the developer.

WorkbenchFX comes with an out of the box styling, a good user experience and builds a house around the views.
It also scales depending on growing requests.
In the beginning, a developer just wants to navigate through the views, but afterwards he'd probably like to use a menu or a toolbar.
Even that is supported by WorkbenchFX and he don't have to build anything by himself.
If everything fails and the blanket falls on his head, he can even replace whole parts of the workbench with own implementations, without having to rewrite the whole workbench.

Thinking of a home, the rooms, the decor, or good memories come into mind.
...
Thinking of developing a new software, a developer thinks about the views, the features and not about on how to switch between those views, or the navigation in general.

With WorkbenchFX it is possible to focus on designing the rooms and meanwhile WorkbenchFX is building the house around it.
This house even adapts with growing requirements without having the developer to do anything.

The paradigm of WorkbenchFX is: "Make everything possible. Expect the least."

Some of the used terms are explained in <<terms-declaration>>.

.Declaration of used terms during the document
[#terms-declaration]
|===
|Term        |Explanation
|Module      |The workbench object consists of multiple views. The object which contains the view is called Module. (More information about <<_module_naming>>)
|Implementor |The person which creates a custom implementation of a module
|User        |The person which uses the final software created with WorkbenchFX
|API user    |The person which uses the WorkbenchFX API and all its features to develop the software
|===

== Motivation
// TODO: Probably more about the Vision?
Our customer told us about his latest project in which he creates a larger enterprise application.
During the conversation he stated that he has to often program the same code sequences again and again to produce the same results.
Larger enterprise applications share often similar functionality and therefore duplicate work is inevitable.
Therefore the idea came up to create an API which allows the user to focus on the unique, interesting work and does all the boring, repetitive work for him.

== Analysis

=== Persona
During the project, three persona have been created.
They help a lot to focus on the main idea of the project.
Thanks to those persona, we were also able to reveal hidden requirements.
For example in the beginning of the project we had only two persona.
But after the first meeting with our customer, we discovered together a third, which lead to different requirements.
The final three persona are shown in <<persona>> below.
We'll refer to their names in the documentation.
...
=== Applications
One of our fist steps in the project was gathering information about existing applications.
Because there are lots of applications available, we tried to look at a variety of them to gain a good overview.
...
==== Blender
Blender is a tool for creating 3D models which are used in animation movies or in game development.
It is a tool which requires a lot of skill and is not an "every day" program.

<<img-blender>> below gives a short impression of the program and <<characteristics-blender>> after shows our findings.
...
- Center: The model which is created (main work)
|
|
|All sections are resizable
|
|If their size becomes 0, they disappear.
 They can be restored, but only by using a short code
|The sections can be shown and hidden as they are not used anymore
...
(probably give a setting to turn this feature on and off)
|When a user is used to work a certain way he has to change the workspace every time he starts a new project
|===

==== Photoshop / Illustrator
Adobe Photoshop and Illustrator are probably the most widely used tools when it comes to photo manipulation (Photoshop) and creation of vector art (Illustrator).
Both tools are made for power user and require a little experience for using them.
Thus both tools are very similar, they are analyzed in one chapter.
<<img-photoshop>> below gives a short impression of the program and <<characteristics-photoshop-illustrator>> after shows our findings.

.IntelliJ IDEA
[#img-photoshop]
image::include/analysis/photoshop.png[Adobe Photoshop]

.Characteristics of Photoshop / Illustrator
[#characteristics-photoshop-illustrator]
|===
|Characteristics |Positive Impressions |Negative Impressions
|The workspace is divided up into five sections:
...
- Center: The files which are worked on
|
|
|- Multiple windows possible

- Navigation is done by tabs
|Tabs are saving space and show still tho open files
|
|Tabs can be navigated to other places (even "un dock" them from the tab bar)
|Sometimes useful when wanting to see two views next to each other
...
|It has advantages, but is also a little diffuse to use. This feature needs for sure to be discussed
|The workspace can be restored to default using the equivalent setting button
|Good solution
|
...
<<img-intellij>> below gives a short impression of the program and <<characteristics-intellij>> after shows our findings.
...
|===
|Characteristics |Positive Impressions |Negative Impressions
|The workspace is divided up into six sections:

- Header: A menu bar with all the different settings available

- Toolbar: Buttons which are often used during development of the product
...
- Bottom bar: Additional space for drawers and tools. Widely used for terminal inputs
...
|
|
|All possibilities available, regarding the tab:

- Right click on the tab, then one can decide its behaviour
|Nearly every state is supported
...
- ...
|
|The "Remove from sidebar" button removes the targeted feature from the sidebar
|
...
-> Lesson learned: Layout changes should be stored at one place
|
|
|
|===
...
<<img-figma>> below gives a short impression of the program.
...
So it can definitely being used by non power users.
The image below gives a short impression of the program and the table below shows our findings.

<<img-Adobe-xd>> below gives a short impression of the program.
...
* A menu bar on top of the application, where all the functionality of the application is stored.
* Below the menu-bar is often a toolbar, which contains the current, or most important tools represented through buttons (without collapsing).
* In center is often the window, in which the main work is done.
...
During our research we discovered, that there are possibly two different kind of application types, created for two different types of users.
...
*** Window mode
*** Split mode
* The application requires a modular approach to satisfy the needs of the user

The other user type is the opposite to a power user:
A user which has less or no experience in using a large application:
...
=== User Stories
To determine the approach of WorkbenchFX, we decided to make a workshop with our customer.
In this workshop we worked with user stories.
This way we could instantly verify if a feature was really necessary and useful or not, because the story needed to fit in the context and was not just a random requirement which came in mind, without verification.

The user stories were especially effective, because during the meeting we discovered an additional persona which changed the whole purpose of our project.
At first we thought, that it would be sufficient to design an API for a long time JavaFX programmers (SteffiFX), but with the third persona (Fabian) we had to especially keep an eye on developers with rare or no experience in frontend development.

==== IDE Decision
During the user stories meeting it was also decided which application type we would support.
As it became clear that our third persona was going to be Anna, we knew that we would not develop an API which allows to build large power user applications like an IDE.
...
// TODO: Include resource reference?
To ensure shared understanding between the customer and ourselves, we agreed on a minimum viable product (MVP).
The outcome resulted in a very minimalistic version of the workbench, which still provides an added value comparing to writing it manually.
The final prototype is displayed in <<mvp>> below.

The MVP consists of:

* Able to add modules to the workbench object
* The <<_workbenchmodule_lifecycle>> which enables the user to open, deactivate and close the module
* Switching modules using tabs
...
NOTE: It is interesting, that the MVP is still recognizable when using only the minimal feature of WorkbenchFX: Adding just modules to the workbench. The outcome represents exactly the MVP.
...
=== Usability Tests
During the project we continuously got feedback from different kinds of people.
In the following chapters are the most important outcomes from usability tests listed.
...
This showed us some weaknesses in the API.
Two very important findings were discovered:

===== Interrupt Closing a Module
It is possible to interrupt closing a module.
For instance if someone wants to ask the user if the progress should be saved before closing, the module should not be closed immediately.
During the implementation of the dialogs we used `CompletableFutures`.
we used them also to achieve the interrupt closing process since a dialog would be shown in our example.

First it made sense and it also worked.
But during the usability test our advisor mentioned the complexity of using `CompletableFutures`.
It is also unlikely, that Fabian would know what `CompletableFutures` are and we didn't wanted him to have an unnecessary barrier to achieve his goal.

So we figured out a much simpler solution.

NOTE: A detailed chapter about interrupt closing a module can be found in the readme.md of WorkbenchFX.

===== Module Toolbar
Our advisor's module had a dark background, but the tab's color was white.
This lead to an inconsistent coloring and destroyed the tab like look and feel.
Having to restyle everything manually was tedious and boring, so we came up with a different idea.

Since the most modules would have some toolbar items we introduced the module toolbar.
It is now possible to add `ToolbarItems` to the module.
As soon as there are items to be displayed, WorkbenchFX creates automatically a toolbar and puts it below the tab.

This way it is ensured, that an implementor always has the possibility to maintain a consistent look and feel.

===== Drawer Animations
During a long period of time of the project we didn't focus on animations.
One reason we never did that, is that using them in `JavaFX` is really cumbersome.

One of our features are the drawers.
An API user can call `workbench.showDrawer()` to show a drawer.
The base idea of this feature is to create a custom `NavigationDrawer`.
When showing or hiding ,those drawers were never showing any animation.
They just (dis)appeared.

However during the usability test we discovered that in case of drawers animations are required.
The visual feedback was not there when we tested the drawers.
A user wants to know from which position the drawer comes from and he is used to a sliding animation.
So we decided to implement the drawers with animations (see <<_animations>>).

==== Manuel Cottone, UX Expert
The biggest impact on the WorkbenchFX styling came from this UX expert.
A lot of the feedback he gave us was really helpful and we decided to adapt a lot of it.

// TODO: reference the material design?
* The active tab should be more styled like a real tab in a browser (at this time our active tabs had simply a white border on bottom of the tab https://material.io/design/components/tabs.html[Material Design]).
...
* Shrink down the size of the close icon in the tabs
* A small vertical border before and after the tabs would suggest the tab as an entity
* The last opened tab could be still open when restarting the application (this can already be achieved through storing the open modules in preferences. For example using PreferencesFX)
* A standard tab could be opened when starting the application (this can already be achieved by simplay defining one in the code before the `launch()` call)
...
However during her usability test it was confirmed that separating the toolbar from the tab bar was really necessary.
Having only one bar lead really fast to an overloading of items and tabs.

===== Introducing ToolbarItems
As an addition to the overloaded bar on top, she was confused with the possibility of adding every `Node` to the toolbar.
She also didn't recognized the Dropdown feature we were providing.

For those reasons we decided to restrict the possibility of adding every `Node` to one simple object:
The `ToolbarItem`.
We replaced the Dropdown with this universal object.
The idea behind the `ToolbarItem` is, that depending on its stored parameters it becomes either a `Label`, a `Button` or a `MenuButton`.
If an API user still wants to add a custom `Node`, he can use the constructor which takes a `Node` as single parameter.
...
Those modules can be checked out when launching the `CustomDemo` in the `workbenchfx-demo` package.
This way, all the features can be seen exactly in details and in action.

=== Layout
Following chapters cover some of the most interesting topics and design decisions we encountered during the layout process.

==== Tabs
We stuck early to the modular concept of having multiple views and switching between them using tabs.
...
For example smartphone operating systems use such a concept, when they display icons of installed apps and open them on click.

For this reason we decided to go with a system, where the user could click on a certain button to open a module.
After clicking on this button, a page would open on which all the available modules are displayed on a grid.
Clicking on one of those "Tiles" (since the rectangular button to open the module looks like a tile, we agreed to go with this name) would open the module.
This way, the user has a good overview on all the modules and could easily open them.

Going with this concept triggered a discussion about naming the components.
When we described the concept, we struggled with naming the module.
...
Fabian for instance would be very confused when he should implement a "MiniApplication".
So we had to find a better name.
In the end we came up with WorkbenchModule which was also not the best solution, since a lot of associations stick with the name module.
But it is definitely a better choice than "Application" (the complete naming process is described in <<_module_naming>>).

When having a lot of modules, the available space on the page would fast shrink and the usability would also decrease.
...
* The phase after the usability tests, where we were <<_separating_the_toolbar, separating the toolbar>> into the toolbar on top and the tab bar below and went full material

===== Writing our own stylesheet
// TODO: cite the material design guideleines
In version `v1.1.1` we had the opinion that we're having a really good styling.
...
Later on during the project we realized, that the white gaps between the toolbar items on top didn't provide a good usability.
It also took too much space.
At one point we decided to have a look at common style guidelines to improve our styling.

We felt, that Google's `Material Design` guidelines are a pretty good starting point since their flat design approach is being used a lot in applications and they are providing the guidelines open source (see https://material.io/design/[Material Design]).
...
We also changed the way the active tabs were looking to a more material like appearance with a white border below the active tab.
...
// TODO: cite the material design guideleines
The way those tabs are styled adapts mainly to tabs with only one single text.
Since we're having an icon, a text and a close icon there was a lot going on in the toolbar.
Especially when having a lot of modules open, the application looked stuffed.
...
[TIP]
====
Therefore we came to conclusion, that testing boundaries is not just recommended but crucial to the success of a project.
...
// TODO: cite the material design guideleines
At first we were worried that separating the toolbar into two bars would take too much space of the window.
...
An example of the created illusion is showed below (<<illusion>>).
...
==== Easy Styling
// TODO: source reference of using variables and source of variable naming of MD
Our superior goal was to enable Fabian a simple way to style his application in a very simple way.
...
We went with the idea of Fabian changing only a few color values and the whole application would be restyled.
To achieve this we worked with css variables.
We defined all of the color variables in the `* / .root` selector of the `css` file and used throughout the whole stylesheet only those variables.

During defining the color variables, we discussed their names and usage in the workbench.
In the end we decided to rely on the Material Design naming convention of colors and to use them as defined.
This way, every API user is able to research on how to use our variables and in which context they are used.
...
But we were not finding much information about using it with `JavaFX` projects.
But because `SCSS` is:
...
If we want to change the colors on hover we had to write following `CSS` code:
...
The code appears to be redundant and very flat, bad to read.
...
Is much easier to read thanks to the nested code writing.
The code is also adaptable.
If additional stylings are required, they can simply be added.
`SCSS` will become handier and more powerful as the complexity of the stylesheet grows.
...
The most mentionable are described as follows.

===== Using the Material Design Variables
// TODO: reference the variables
As stated in <<_easy_styling, Easy Styling>> we wanted to have Fabian worrying about restyling as least as possible.
...
Of course not only Fabian profits from this convention but we too.
Thanks to variables we didn't had to worry about color codes and changing them.

Using the variable names and usage definition of `Material Design` will also be of use in future.
This way we have not to worry about other, future developers not knowing on how the colors are used and how to use them.

===== Replacing Pixels
We agreed on using `em` as size measurement and not `px`.
The simple reason to do this was agility.
When changing the default size of the font, the overall styling can adapt the actual size.
...
Following <<best-practices>> lists all the conventions we consider the most valuables from this project.

.Best Practices
[#best-practices]
|===
|Convention        |Reason
|Maintain code review |It worked very well in PreferencesFX, so we will keep it that way
...
|Using build tools |It makes the development much easier since we don't have to worry about running tests, checking checkstyle and javadoc.
|Committing more often |It makes the development process more transparent. When problems occur it can easily be reverted
|Work with persona |It helps to focus on the target group. This way the development process can always be readjusted
...
User stories communicate a concrete goal of the product.
Comparing to just defining some features, user stories help to understand the reason for a feature.
This way we can ensure that all of our implemented features satisfy a special need.

In the beginning of the project we held a workshop with our customer where we defined the persona and afterwards the user stories to define the problems which our API should solve.
Before the workshop it was rather unclear what WorkbenchFX exactly is.
But after the workshop we all had a shared understanding about the project.

Together with the persona and the user stories it felt almost like the written stories and persona are real.
Exactly to achieve this feeling is what are user stories made for.

During the project we could always refer to the persona and stories and ask ourselves:
"Are we on the right way?" It helped us a lot to keep an eye on the real problem and to not loose ourselves in personal opinions and thoughts.

[TIP]
====
For a future project we would definitely use persona and user stories again.

Having three different persona is tricky to work with as we have to look after three different characters and needs.
Of course having that much persona can happen but having only one central persona is crucial since we cannot always satisfy every desire and therefore have to make sometimes compromise.
====
...
=== Hacking Day
During the project we had several so called "Hacking Days" where we together with our customer and/or advisor worked on the project.
Those hacking days completely differ from each other.
...
Thanks to those kind of hacking days we discovered several issues which we could eliminate then.

Another hacking day was held with our customer.
There we started implementing together the dialog feature.
...
He stated for instance that is was not that easy as he thought to simply reuse his code of the dialog.

However in the end we all profited from each other.
We are really grateful about those hacking days since they really are helpful.
