...
While it is not recommended to open an overlay on top of another overlay, it frees the API user from having to account for timing / concurrency issues during transitions between overlays.

==== Discarded Implementations


At first, we implemented overlays
