...
However, as stated in chapter link:#add_module_page[Add Module Page] the name `Module` is not ideal, since that name already has a lot of associations.
...
==== Dieter Holz, Advisor
During the "Hacking Days" we've made from time to time (more about them in chapter <<_hacking_day>>), our advisor tested the API by implementing a module on his own.
...
Most of the applications analyzed in chapter <<_applications>> use the tab concept to switch between their files.
...
But it is definitely a better choice than "Application" (the complete naming process is described in chapter link:#module_naming[Module Naming]).
...
* The phase when we were <<_writing_our_own_stylesheet>> without any help of external sources.
* The phase where we were <<_switching_to_the_material_design_guidelines>>.
* The phase after the usability tests, where we were <<_separating_the_toolbar>> into the toolbar on top and the tab bar below and went full material
...
As stated in chapter <<_easy_styling>> we wanted to have Fabian worrying about restyling as least as possible.
