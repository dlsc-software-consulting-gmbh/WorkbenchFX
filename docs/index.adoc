...
== Abstract
*WorkbenchFX* is an open source Java based JavaFX framework designed to create large enterprise applications.
...
== Acknowledgements
We had a lot of help on our way to *WorkbenchFX*.
This project would not have been so successful without the help of all of the people who supported us.
...
Giving us constant feedback, he helped a lot carrying the project forward.
...
We would also like to especially thank the UX expert, who supported us during the usability test phase.
His feedback led to a substantial improvement of our styling and user experience.
...
Some of the terms used are explained in <<terms-definition>>.

.Definition of terms used during the documentation
[#terms-definition]
|===
|Term        |Definition
...
|Implementor |The developer who creates a custom implementation of a module
|User        |The person who uses the final application created with WorkbenchFX
|API user    |The developer who uses the WorkbenchFX API to develop their application
|===

== Motivation
Our customer told us about his latest project in which he creates a large enterprise application.
...
This helped us decide which characteristics we want to include in WorkbenchFX.

Because there are lots of applications available, we tried to look at a variety of them to ensure a good overview.

The following subchapters show the analyzed applications, with an image, a table with the characteristics and if there are, positive or negative impressions each.
...
|- There are no animations when showing or hiding
...
|When a user is used to working in a certain way, they have to change the workspace every time they start a new project
|===

==== Adobe Photoshop
Adobe Photoshop is probably one of the most widely used tools when it comes to manipulation of photos and creation of digital art.
...
|They have 3 states (not likely to understand) and the collapsing behavior is not intuitive
|===
...
- Bottom bar: Additional space for drawers and tools. Widely used for additional collapsible drawers
...
|
|
|When right clicking on the tab, its positioning behavior can be defined
...
==== Figma
Figma is a tool used for prototyping of user interfaces.
...
* Menu bar on top, where all the features are placed
...
==== Conclusion
After analyzing all the different programs, we simplify them to the most important and frequent characteristics:

* A menu bar on top of the application, where all the functionality of the application is located.
* Below the menu bar is often a toolbar, which contains the current, or most important tools represented by buttons (not collapsible).
* The window, in which the main work is done, is often in the center.
...
* Finally another toolbar is sometimes set on the bottom of the application.
It contains the least used tools, or tools which are needed in the end of the process.

==== To IDE or not to IDE?
During our research we discovered that there are two different kinds of application types, created for two different types of users.

One user type are power users:

Power users need the utmost of customizability, to ensure they can adapt the workspace to be the most ergonomic to fit their workflow, like in an IDE (Integrated Development Environment).
...
The other user type is the opposite of a power user:
...
The purpose was to define personas (see <<_persona>>) and to create user stories based on them.
...
____
...I'd like to have an intuitive way of using the application...
____
concluding with the value:
____
...so that I'm not confused while working and to have a productive workflow.
____

This way, we can instantly verify if a feature is really necessary and whether it is useful or not.
This is because the story needs to fit in the context and is not just a random requirement which came to our mind, without questioning its usefulness cite:[cohn_user_2004].

==== IDE Decision
During the user story workshop (see <<_user_stories>>) we also decided which application type we would support.
...
She doesn't want to bother with moving parts of the GUI around, she just wants to get her work done quickly with good user experience out of the box.
...
* The <<_workbenchmodule_lifecycle>> which enables the user to open and close the module
...
==== Dieter Holz, Advisor
During the "Hacking Days", we've had from time to time (more about them in <<_hacking_day>>), our advisor tested the API by implementing a module on his own.
...
Having Fabian to restyle everything manually is tedious and boring, so we came up with a different idea.
...
A lot of the feedback he gave us was really helpful and we decided to adopt a lot of it.
...
* Offer the ability to have the same tabs open as when the application was last closed (this can already be achieved through storing the open modules in preferences, for example using https://github.com/dlemmermann/PreferencesFX[PreferencesFX])
...
When she looked at the top bar during the usability test, she thought all controls in the bar were tabs and was confused what adding controls to the lists of the toolbar controls would do.
This confirmed that separating the toolbar from the tab bar was really necessary.
Having only one bar quickly led to an overcrowding of items and tabs, which made it impossible to distinguish them.

===== Introducing ToolbarItems
Additionally to the overcrowded bar on the top, she was confused because of the possibility of adding all objects of type `Node` to the toolbar.
She also didn't realize the `Dropdown` control we were providing for convenience.

For those reasons, we decided to restrict the possibility of adding objects of type `Node` to one simple object:
The `ToolbarItem`. +
...
== Implementation
During development of the API we built a lot of demo modules to test it.
Those modules can be tried out by launching the `CustomDemo` in the `workbenchfx-demo` package.
This way, all features can be seen in detail and in action.
...
For example, smartphones use the same kind of concept when they display icons of installed apps on the home screen and open them on click.

For this reason we decided to go with a system, where the user can click on a certain button, which opens an overview with all of the modules shown in a grid.
Clicking on one of the modules, represented as buttons (called `Tile`) opens the corresponding module.
This way, the user has a good overview on all the modules and can easily open them.

Going with this concept led to a discussion about the names of the components.
...
For obvious reasons we could not name the module `MiniApplication` or even `Application`.
...
In the end we chose `WorkbenchModule`, which also was not the best solution, since the name "module" has a lot of associations.
But it is definitely a better choice than "Application" (the complete naming process is described in <<_module_naming>>).

When there are a lot of modules, the available space on the page would shrink quickly and the usability would also suffer because of the buttons becoming too small.
For this reason, we introduced a pagination to only have a restricted amount of modules displayed on a page.
...
In version `v1.1.1` we thought the styling looked great.
...
We also changed the way the active tabs were looking to a more Material Design like appearance with a white border below the active tab cite:[noauthor_tabs_nodate].
...
However, the styling of the active tab made us realize that Material Design *is* only a guideline.
The way the tabs are styled applies only to tabs with *just* text cite:[noauthor_tabs_nodate].
...
<<v150>> below shows how horrible the styling looked with a lot of modules open.
...
[TIP]
====
We came to the conclusion, that testing *edge cases* is not just recommended *but crucial* to the success of a project.
...
It is obvious this styling had to change.
...
A direct comparision supports this decision (<<v150vs160>>).
...
An example of the illusion can be seen in <<illusion>>.
...
==== Easy Styling
Our most important goal was to enable Fabian to style the workbench in a simple way.
...
We wanted to have Fabian only change a few color values to completely restyle the whole application consistently.
To achieve this, we worked with `CSS` variables.
We defined all of the color variables in the `.root > *` selector of the `CSS` file and only used those variables throughout the whole stylesheet.

During the definition of the color variables, we discussed their names and usage in the workbench.
In the end, we decided to go with the Material Design naming convention of colors and use them as defined cite:[noauthor_color_nodate].
This way, every API user is able to research on how to use the variables and in which context they are used.

By simply using the color variables, Fabian can create a custom stylesheet, assign it to the workbench and overwrite the colors he wants to change.
...
To account for this use case, we gave every component used in the workbench at least an `#id` or a `.class` name.
This way, SteffiFX is able to restyle the complete application if she likes to do so.

NOTE: A detailed chapter about restyling the application can be found in the `readme.md` file of WorkbenchFX.
...
This would not only make the readability worse, this also means that if we would want Fabian to be able to define the workbench with the least effort possible, we would need to add multiple overloaded `of()` methods.
...
* Require the least amount of effort from Fabian to use it
* Fulfill the needs of SteffiFX, cover as many use-cases as possible
...
However, if we make it too easy, SteffiFX can't cover all of her use cases. +
At the same time, making it more complex potentially makes it less self-explanatory, which in turn makes it harder to use.

The challenge was to strike a balance between complex and easy, ensuring both Fabian and SteffiFX would be happy with the API.
...
This was a bit of a challenge itself though, since we initially thought it would be enough to have `void` as the return type.
...
In case of a confirmation dialog before closing the module, the implementor can simply return `false`, causing the closing of the module to get interrupted (for more detail, see <<_workbenchmodule_lifecycle>>).
...
It also eliminates the "guessing" of how it was implemented, as can be observed mentioned above in the possible open questions.

==== Icon: Node vs File vs Image
To specify an icon for a `WorkbenchModule`, there are different ways to implement this in the API.

===== Node
The most generic option would be to have the API user pass in a `Node` to the constructor to use as the icon.
...
Since the icon will always be the same for the `Tab` and the `Tile`, this feels redundant and is very error-prone.
...
===== Image
We decided to use `Image` as one of the options of passing in an icon to the constructor of `WorkbenchModule`.
...
We thought it would be excessive to use the `Control` and `Skin` architecture from JavaFX for all internal views as well, especially since they don't have to be replaceable (see <<_api_design>>).
...
This simplifies the architecture and readability is improved, since methods are where they are expected.

`View` is an interface, because it's being used as a mixin.
...
Since all presenters don't need to extend anything else, using an abstract class is possible.

==== Overlays
To enable the API user to easily show a custom overlay with a black transparent `GlassPane` in the background, they can use the respective `workbench.showOverlay()` and `workbench.hideOverlay()` methods.
...
=== Development
During the development, we made extensive use of GitHub's features.
...
This worked very well for us, since comments that are made by the other person always lead to improvements and code of higher quality.
...
* easier to read
* requires less code the more complex the styling becomes
* provides many more features compared to `CSS`

We decided to give it a try.

For example, in an application with two views in one larger view.
...
The code appears to be redundant and is not very readable.
...
It is much easier to read thanks to the nested code.
The code is also more maintainable.
When additional styling is required, it can simply be added.
`SCSS` becomes more useful and powerful as the complexity of the stylesheet increases.
...
The most important are described in the following subchapters.

===== Using the Material Design Variables
As stated in <<_easy_styling, Easy Styling>>, we want to offer Fabian an easy way of restyling the workbench.

This is why we agreed on using only variables for styling, since he then only has to change the variable's values to restyle the application.
Of course, not only Fabian profits from this convention but we do so too.
Thanks to variables, we didn't have to worry about color codes and changing them internally.

Using the variable names and Material Design will also be of use in the future.
This way we don't have to worry about future developers not knowing how the colors are used cite:[noauthor_color_nodate].
...
We wrote a little <<pixel-converter>> which allows us to convert pixels into `em` and agreed on using the function when working with a pixel value.
[source,sass]
...
==== JUnit vs Spock
Most of our unit testing, especially in the beginning, was done with JUnit.
...
At first, we proceeded to write the data-driven tests as usual with JUnit.
But the tests had a lot of of duplicated code and therefore were not very maintainable (see <<comparison-junit-spock>>).
So we decided to invest some time to find a better solution.
...
However, it was still unclear whether we could use Spock to test Java code and even more so JavaFX applications, which are already more difficult to test in our experience.
In theory it should be possible, as Spock claims on their website cite:[noauthor_spock_nodate].
Also, TestFX (see <<_testing_utilities>>) offers support for Spock, so it seemed like a viable option.
...
We rewrote the data-driven JUnit 5 test with a lot of duplicated code mentioned above with Spock and compared both in <<comparison-junit-spock>>.
...
To remedy this issue with TestFX, we noticed that it is necessary to wrap the calls on the GUI thread in a `interact()` block, as can be seen in <<testfx-testing>>.
...
.Minimal example of testing JavaFX with TestFX
[[testfx-testing]]
[source,java]
----
class JavaFxTest extends ApplicationTest {
  ...
  private Label nodeUnderTest

  @Override
  public void start(Stage stage) {
    robot = new FxRobot();
    nodeUnderTest = new Label("Test");

    Scene scene = new Scene(nodeUnderTest, 100, 100);
  ...
  void test() {
    robot.interact(() -> {
      // calls on the GUI thread of JavaFX need to be performed here
      assertEquals("Test", nodeUnderTest.getText());
    });
  }
}
----
...
Travis makes two checks each time there is a push to the repository.
...
In the end, we were able to reach a code coverage of *94.51%*.
...
This was the case mainly when a new major feature was introduced with a pull request and there are no other major features expected in the next few days.

Concerning the version numbering scheme, we chose to adhere to the following numbering scheme: `vX.Y.Z`
...
==== Release Automation
We also automated parts of the release process with Travis.
...
The only thing left is to edit the title and description with the release name and its changelog, respectively (see <<_process>>).
...
== Lessons learned
Before starting the project, we did a retrospective on our previous project https://github.com/dlemmermann[PreferencesFX].
We agreed on maintaining the processes which went good and defined new conventions to do better in this project.
<<best-practices>> lists all the conventions we consider the most valuable in this project.
...
|Writing the Javadoc before merging |In PreferencesFX we had to write nearly all the Javadoc in the end of the project
|Merging only when checkstyle passes |Like the Javadoc, we had to fix a lot of checkstyle error in the end
|Writing the documentation early |If we write the documentation in the end, we have the problem of remembering all the content half a year later.
This would lead to details getting lost.
Additionally, we have no time pressure at the end of the project
|Writing more tests |Since we're writing an API, it's necessary to write more tests to maintain better code quality and less errors
|Using build tools |It makes the development much easier, since we don't have to remember to run tests, review checkstyle and javadoc on our local machines during code review.
...
|Work with user stories |It helps to verify the features and reveals the needs of the users which WorkbenchFX has to satisfy
|===

=== Value of User Stories
As stated in <<_user_stories>> we had a user story workshop with our customer in the beginning of the project.

Before the workshop, it was rather unclear what WorkbenchFX or its goals really were.
...
It helped us a lot to keep an eye on the real problem and to not lose ourselves in personal opinions and thoughts.
...
Each hacking day was completely different from the others.

When we had the hacking days together with our advisor, he assumed the role of Fabian and implemented his own module.
...
We held another hacking day with our customer.
...
We are really grateful about those hacking days, since they are really helpful and would like to do them again in the next project.
...
After implementing the drawer animations, we were astounded seeing how much of a difference it made.
...
This is why we switched WorkbenchFX to use Maven as well.
...
=== Build Automation
The automated build we set up with Travis CI was very useful, in fact it would have been even more useful if we had set it up from the beginning.
...
We finished the project in a way that we can say that we achieved all of our goals we set in the beginning.

Besides starting to write the documentation early in the project, we managed to adhere to all of the <<_lessons_learned,conventions>> we defined in the beginning of the project.
They proved to be worthwhile.
...
Also, SteffiFX is able to replace a lot of features with her own implementations (see <<_constructing_the_workbench>>).
Anna is satisfied because she gets an application which is well styled and not overloaded with a lot of functionality.
...
In the end, we were even able to reach a code coverage of *94.51%* (see <<_processes>>).

All of the feedback we received up to now is really good.
Even at the bachelor exhibition a lot of people stated that they really like the idea and that they would be interested in trying out *WorkbenchFX* cite:[noauthor_ausstellung_nodate].
...
* Implement lazy-loading of tabs when opening them on start of the application, so the thread is not blocked for a long time
