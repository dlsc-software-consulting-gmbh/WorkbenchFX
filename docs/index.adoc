...
See the chapter <<_add_module_page>> for the explanation of what a module is.
...
We chose the MVP pattern because we used in in the https://github.com/dlemmermann/PreferencesFX[PreferencesFX] project and it proved to be very good for those purposes.
...
Since we had a focus on good "usability" of the API for the API users, the API would change a lot during development.
...
// TODO: add source and maybe code snippet of data-driven testing with junit?
JUnit 5 does have data-driven testing capabilities, but it doesn't offer much flexibility and the way tests have to be implemented to be data-driven is not very readable.
// TODO: add source
Also, JUnit 5's data-driven testing API is still considered "experimental".
...
// TODO: very well implemented => source?
However, Spock has one big advantage over JUnit: Data driven testing is very well implemented.
Parameters in data-driven tests are implemented in a table-like format.
// TODO: fully parametrized??
A test case can be built fully parametric, which means the tested method and assertions only need to be written once.
Spock will then dynamically run the test case multiple times with each row of the table-like data structure as parameters.
...
// TODO: add source
A quick research proved that in theory it could be possible.
Even TestFX (see <<_testing_utilities>>) offers support for Spock, so it seemed like a viable option.
We did some research in GitHub, but we didn't find any Java projects which used *JUnit 5* together with Spock.
After a lot of challenges, we were able to make it work.
// TODO: source for junit vintage => junit 4
The key part to the solution, was that we needed to add the JUnit Vintage Engine, which is the engine from JUnit 4.
...
// TODO: source for toolkit not initialized, exact message
Without it, JavaFX will throw `Toolkit not initialized` exceptions for all method calls that run on the GUI thread.
...
This was quite a challenge, since things like `Thread.sleep()` didn't help at all, as they probably were just sleeping the GUI thread, which didn't help with the issue.
